...
func main() {
	...
	// We can use multiple cases above the default clause to implement a multi-way non-blocking select. Here we attempt non-blocking receives on both messages and signal
	select {
	case msg := <-msgs:
		fmt.Println("Received message", msg)
	case sig := <-signals:
