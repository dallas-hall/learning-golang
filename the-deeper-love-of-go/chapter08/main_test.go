...
func TestSaveCatalogue_SaveCatalogueDataToFile(t *testing.T) {
	t.Parallel()
	err := books.SaveCatalogue("test-data/catalogue2.json")
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestOpenCatalogueAndSyncCatalogues_ReadDataWrittenBySync(t *testing.T) {
	...
	path := t.TempDir() + "/catalogue-test-data.json"
	err := catalogue.SyncCatalogues(path)
	if err != nil {
		t.Fatal(err)
	}

	// Test reading catalogue from disk
	newCatalogue, err := books.OpenCatalogue(path)
	if err != nil {
		t.Fatal(err)
	}
